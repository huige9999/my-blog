---
title: 用 Worktree 把本地并行开发变简单
date: 2025-11-13
description: 如何用 Git Worktree + Claude Code 实现安全的本地并行开发，避免切换分支打断进行中的任务。
---

# 用 Worktree 把本地并行开发变简单

## 痛点：Agent 在跑,我却想并行干别的

我经常让 Claude Code 在当前分支上跑长任务：重构、批量改动、生成代码、跑测试……任务执行过程中我突然想插入另一个小任务（修 bug / 看日志 / 改配置 / 做实验）。但我不敢切分支：
...
这个"最终决定"非常有亮点：不是记命令,也不是写大 Prompt,而是**把命令记忆外包给 agent**,我只保留"原则和验收"。

一句话概括：

> **"我不记命令,我只记模型：worktree 是什么、它会影响什么、如何验证。"**

## Worktree 原理：它到底解决了什么问题？
...
## 我的最终工作流：自然语言驱动的"小步执行"

核心是：**你只下达意图,agent 输出命令,你执行或让它执行。每步可验证。**

### 创建一个新 worktree（我只说目标）
...
- **端口冲突**：两个 worktree 同时跑 dev server 时会争抢端口
- **构建输出互相覆盖**：确保构建缓存和输出目录放到各自 worktree 内,而不是共享路径
